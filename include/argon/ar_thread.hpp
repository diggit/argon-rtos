#if !defined(_AR_THREAD_H_)
	#define _AR_THREAD_H_
...
namespace Ar {
	...
	class Thread {
	  public:
		...
		Status m_unblockStatus {Status::success}; //!< Status code to return from a blocking function upon unblocking.
		...
		struct ThreadStatus {
			...
			constexpr ThreadStatus(
				...
				m_thread(thread),
				m_name(name), m_uniqueId(uniqueId), m_cpu(cpu), m_state(state), m_maxStackUsed(maxStackUsed), m_stackSize(stackSize) {};
			...
			constexpr ThreadStatus(Thread *thread, const char *name, std::uint32_t uniqueId, std::uint32_t cpu, ThreadState state, std::size_t maxStackUsed,
				...
				m_thread(thread),
				m_name(), m_uniqueId(uniqueId), m_cpu(cpu), m_state(state), m_maxStackUsed(maxStackUsed), m_stackSize(stackSize) {
		...
		//!     #kArSuspendThread can be used to better document this parameter value.
		template<class T>
		Thread(const char *name, T *object, void (T::*entry)(), void *stack, std::size_t stackSize, Priority priority,
			InitialState initialState = InitialState::running) :
			Thread(name, object, member_thread_entry<T>, stack, stackSize, priority, initialState) {
		...
		//!     #kArSuspendThread can be used to better document this parameter value.
		template<class T>
		Thread(const char *name, T *object, void (T::*entry)(), std::size_t stackSize, Priority priority, InitialState initialState = InitialState::running) :
			Thread(name, object, member_thread_entry<T>, nullptr, stackSize, priority, initialState) {
		...
		Status init(const char *name, Entry entry, void *param, void *stack, std::size_t stackSize, Priority priority,
			InitialState initialState = InitialState::running);
		...
		//! @retval #kArOutOfMemoryError Failed to dynamically allocate the stack.
		template<class T>
		Status init(const char *name, T *object, void (T::*entry)(), void *stack, std::size_t stackSize, Priority priority,
			InitialState initialState = InitialState::running) {
	  ...
	  protected:
		...
		static void member_thread_entry(void *param) {
			...
			std::memcpy((char *)&member, storage, sizeof(member));
			(obj->*member)();
		}

		//! @brief Special init method to deal with member functions.
		Status initForMemberFunction(const char *name, void *object, Entry entry, void *memberPointer, std::size_t memberPointerSize, void *stack,
	  ...
	  private:
		...
		Ar::Status resume_internal();

		Ar::Status suspend_internal();
	}; // namespace Ar
	...
	class ThreadWithStack : public Thread {
	  public:
		...
		//! @brief Constructor to set the thread entry to a member function.
		template<class T>
		ThreadWithStack(const char *name, T *object, void (T::*entry)(), Priority priority, InitialState initialState = InitialState::running) :
			Thread(name, object, entry, m_stack, S, priority, initialState) {};
